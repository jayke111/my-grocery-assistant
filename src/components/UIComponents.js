...
export const Header = ({ onTitleClick, user, onLogoutClick, onLoginClick }) => {
    // --- ADDED: Get setPage from context to navigate ---
            ...
            <div className="absolute top-4 right-4">
                {user ? (
                    <div className="flex items-center gap-x-4">
                        {/* --- MODIFIED: Added the Profile button --- */}
                    ...
                    <button onClick={onLoginClick} className="bg-blue-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-blue-700 transition">Go Pro ✨</button>
                )}
            </div>
        </header>
    );
