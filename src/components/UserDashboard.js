...
export const UserDashboard = () => {
    const { userLists, loadingLists, setActiveListId, handleCreateNewList, handleDeleteList } = useAppContext();
    const [sharingList, setSharingList] = useState(null); // State to manage which list is being shared
        ...
        <div className="space-y-4">
            ...
            {userLists.length === 0 ? (
                ...
                <div className="space-y-3">
                    {userLists.map(list => (
                        <div key={list.id} className="bg-white p-4 rounded-xl shadow-md flex justify-between items-center">
                            <div>
                                <button onClick={() => setActiveListId(list.id)} className="text-lg font-semibold text-gray-800 hover:text-blue-600 text-left">
                                    {list.name}
                                </button>
                                <p className="text-sm text-gray-500">
                                    Created: {new Date(list.createdAt?.toDate()).toLocaleDateString()} &middot; Items: {list.itemCount || 0}
                                </p>
                            </div>
                            <div className="flex items-center space-x-4">
                                {/* UPDATED: This button now opens the share modal */}
            ...
            {/* This will render the modal only when 'sharingList' has data */}
            {sharingList && (
                <ShareListModal 
